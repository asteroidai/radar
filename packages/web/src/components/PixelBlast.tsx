// @ts-nocheck
// Vendored from Asteroid — PixelBlast WebGL background animation
...
const PixelBlast: React.FC<PixelBlastProps> = ({
  ...
  // Set up IntersectionObserver for autoPauseOffscreen
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      ...
      // Initialize threeRef before starting animation so RAF ID can be stored
      threeRef.current = {
        renderer,
        scene,
        camera,
    ...
    return () => {
      // Always abort event listeners
      abortControllerRef.current?.abort();
      abortControllerRef.current = null;

      if (!threeRef.current) return;
      const t = threeRef.current;
      // Always cancel animation frame to prevent leaks
      if (t.raf !== undefined) {
        cancelAnimationFrame(t.raf);
      }
      t.resizeObserver?.disconnect();
